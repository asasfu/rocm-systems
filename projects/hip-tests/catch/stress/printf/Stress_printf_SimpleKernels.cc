...
const struct SizeStruct EmpiricalValues1[12] = {{512, 512, 16},  {512, 512, 32},  {512, 512, 48},
                                                {512, 512, 64},  {512, 512, 80},  {512, 512, 96},
                                                {512, 512, 110}, {512, 512, 126}, {512, 512, 140},
                                                {512, 512, 156}, {512, 512, 172}, {512, 512, 186}};
...
const struct SizeStruct EmpiricalValues2[12] = {{512, 512, 20},  {512, 512, 40},  {512, 512, 60},
                                                {512, 512, 80},  {512, 512, 100}, {512, 512, 120},
                                                {512, 512, 140}, {512, 512, 160}, {512, 512, 180},
                                                {512, 512, 200}, {512, 512, 220}, {512, 512, 240}};
...
__global__ void kernel_printf_variablestr(uint iterCount, int* ret) {
  uint tid = threadIdx.x + blockIdx.x * blockDim.x;
  int retlocal = 0;
  const char* const_str = "Hello World from Device.Iam printing (threadID,number)=";
...
__global__ void kernel_dependent_calc(uint32_t iterCount, int* ret) {
  ...
  int retlocal = 0;
  const char* const_str = "Hello World from Device.Iam printing number=";
  for (int count = 0; count < (const int)iterCount; count++) {
...
// size and 'g' grid size such that
// (total bytes per iteration)*n*b*g ≈ N GB, where N is user input.
__global__ void kernel_dependent_calc_atomic(uint32_t iterCount, int* ret) {
  uint32_t tid = threadIdx.x + blockIdx.x * blockDim.x;
  int retlocal = 0;
  const char* const_str = "Hello World from Device.Iam printing number=";
...
__device__ __host__ int printPi(int maxPrecision) {
  ...
  const char* const_str = "Hello World from Device.Iam printing decimal number=";
  for (int prec = 0; prec <= maxPrecision; prec++) {
    printSize += printf("%s%.*f %.*e\n", const_str, prec, pi, prec, piScaled);
  }
  return printSize;
}

__global__ void kernel_decimal_calculation(uint iterCount, int maxPrecision) {
  for (int count = 0; count < (const int)iterCount; count++) {
...
bool test_printf_conststr(uint32_t num_blocks, uint32_t threads_per_block, uint32_t print_limit) {
  ...
  if ((totalExpectedLines != (totalActualLinecount - 1)) || (expectedFileSize != actualFileSize)) {
    return false;
  }
  return true;
}
...
// and validates the generated file size and number of printed lines
// with the calculated file size and lines.
bool test_printf_two_conditionalstr(uint32_t num_blocks, uint32_t threads_per_block,
  ...
  if ((totalExpectedEvenLines != totalActualEvenLines) ||
      (totalExpectedOddLines != totalActualOddLines) || (expectedFileSize != actualFileSize)) {
    return false;
  }
  return true;
}
...
// and validates the generated file size and number of printed lines
// with the calculated file size and lines.
bool test_printf_single_conditionalstr(uint32_t num_blocks, uint32_t threads_per_block,
                                       uint32_t print_limit) {
  uint32_t iterCount = 0;
  ...
  printf("expectedFileSize = %zu \n", expectedFileSize);
  printf("actualFileSize = %zu \n", actualFileSize);
  if ((totalExpectedLines != (totalActualLines - 1)) || (expectedFileSize != actualFileSize)) {
    return false;
  }
  return true;
}
...
// and validates the generated file size and number of printed lines
// with the calculated file size and lines.
bool test_variable_str(uint32_t print_limit, void (*func)(uint32_t, int*),
...
// and validates the generated file size and number of printed lines
// with the calculated file size and lines.
bool test_decimal_str(uint32_t num_blocks, uint32_t threads_per_block, uint32_t print_limit) {
  // Calculate the number of iterations from print_limit
  size_t stress_limit_bytes = ((size_t)print_limit * 1024 * 1024 * 1024);
  int maxPrecision = 10;
  int totalPrintSizePerIter = printPi(maxPrecision);
  uint32_t iterCount = static_cast<uint32_t>(
      1 + stress_limit_bytes / (num_blocks * threads_per_block * totalPrintSizePerIter));
  // Calculate expected lines of print and file size.
  size_t actualFileSize = 0;
  size_t expectedFileSize =
      (size_t)num_blocks * threads_per_block * iterCount * totalPrintSizePerIter;
  uint32_t totalActualLines = 0;
  uint32_t totalExpectedLines = (maxPrecision + 1) * iterCount * num_blocks * threads_per_block;
  // DO NOT PUT ANY PRINTF WITHIN THIS BLOCK OF CODE
  {
    CaptureStream captured(stdout);
...
// and validates the generated file size and number of printed lines
// with the calculated file size and lines.
bool test_shared_mem(uint32_t num_blocks, uint32_t threads_per_block, uint32_t print_limit) {
  // Calculate the number of iterations from print_limit
  size_t stress_limit_bytes = ((size_t)print_limit * 1024 * 1024 * 1024);
  unsigned total_0_to_blksize = (BLOCK_SIZE - 1) * BLOCK_SIZE / 2;
  char buffer[CHUNK_SIZE];
  int totalPrintSizePerThread =
      snprintf(buffer, CHUNK_SIZE, "%s%u\n", CONST_STR3, total_0_to_blksize);
  uint32_t iterCount = static_cast<uint32_t>(
      1 + stress_limit_bytes / (num_blocks * threads_per_block * totalPrintSizePerThread));
  // Calculate expected lines of print and file size.
  size_t actualFileSize = 0;
  size_t expectedFileSize =
...
// and validates the generated file size and number of printed lines
// with the calculated file size and lines.
bool test_synchronized_printf(uint32_t num_blocks, uint32_t threads_per_block,
                              uint32_t print_limit) {
  // Calculate the number of iterations from print_limit
  size_t stress_limit_bytes = ((size_t)print_limit * 1024 * 1024 * 1024);
  char buffer0[CHUNK_SIZE], buffer1[CHUNK_SIZE], buffer2[CHUNK_SIZE];
  int totalPrintSizePerThread = snprintf(buffer0, CHUNK_SIZE, "%s%u\n", CONST_STR3, 0);
  totalPrintSizePerThread += snprintf(buffer1, CHUNK_SIZE, "%s%u\n", CONST_STR3, 1);
  totalPrintSizePerThread += snprintf(buffer2, CHUNK_SIZE, "%s%u\n", CONST_STR3, 2);
  uint32_t iterCount = static_cast<uint32_t>(
      1 + stress_limit_bytes / (num_blocks * threads_per_block * totalPrintSizePerThread));
  // Calculate expected lines of print and file size.
  size_t actualFileSize = 0;
  ...
  printf("expectedFileSize = %zu \n", expectedFileSize);
  printf("actualFileSize = %zu \n", actualFileSize);
  if ((TestPassed == false) || (expectedFileSize != actualFileSize)) {
    return false;
  }
  return true;
}
...
HIP_TEST_CASE(Stress_printf_IfElseConditionalStr) {
  ...
  // N provide the print limit
  unsigned int print_limit = 1;  // = 1 GB
  TestPassed = hipPrintfStressTest::test_printf_two_conditionalstr(num_blocks, threads_per_block,
                                                                   print_limit);
  REQUIRE(TestPassed);
#else
  printf("This test is skipped due to non linux environment.\n");
...
HIP_TEST_CASE(Stress_printf_IfConditionalStr) {
  ...
  // N provide the print limit
  unsigned int print_limit = 1;  // = 1 GB
  TestPassed = hipPrintfStressTest::test_printf_single_conditionalstr(num_blocks, threads_per_block,
                                                                      print_limit);
  REQUIRE(TestPassed);
#else
  printf("This test is skipped due to non linux environment.\n");
...
HIP_TEST_CASE(Stress_printf_VariableStr) {
  ...
  // N provide the print limit
  unsigned int print_limit = 1;  // = 1 GB
  TestPassed = hipPrintfStressTest::test_variable_str(
      print_limit, hipPrintfStressTest::kernel_printf_variablestr,
...
HIP_TEST_CASE(Stress_printf_AtomicCalc) {
  ...
  unsigned int print_limit = 1;  // = 1 GB
  TestPassed = hipPrintfStressTest::test_variable_str(
      print_limit, hipPrintfStressTest::kernel_dependent_calc_atomic,
      hipPrintfStressTest::EmpiricalValues2);
  REQUIRE(TestPassed);
#else
  printf("This test is skipped due to non linux environment.\n");
