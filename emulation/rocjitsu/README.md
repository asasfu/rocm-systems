...
The per-ISA instruction files under `lib/rocjitsu/src/rocjitsu/isa/arch/amdgpu/`
(decoders, encodings, execute bodies, and the shared execute templates in
`shared/execute_shared.h`) are autogenerated from the MR ISA XML specification,
together with the cross-ISA DBT legalization tables and encoding translators
under `lib/rocjitsu/src/rocjitsu/code/dbt/generated/`.

A single `--multi` invocation regenerates everything: `--gen-isas` and
`--gen-dbt` are both on by default, so one command produces the per-ISA C++, the
shared execute templates, the legalization tables, and the encoding translators.

```bash
export MRISA=$PWD/../../shared/machine-readable-isa/isa
PYTHONPATH=$PWD/lib/python python3 -m amdisa \
  ...
  --multi \
    ...
    gfx1250:/path/to/amdgpu_isa_gfx1250.xml

# Format with the PINNED clang-format (v18) — the system clang-format churns the
# output against the committed v18 formatting.
pre-commit run -c .pre-commit-config.yaml clang-format --all-files
```

**Important — pass ALL ISAs in one `--multi` run.** `shared/execute_shared.h` is
the *union* of shared execute templates across exactly the ISAs you pass.
Regenerating a subset shifts the cross-ISA grouping, drops the templates the
omitted ISAs need, and leaves their (now stale) per-ISA `.cpp` calling undefined
symbols. The build includes `gfx1250`, so it must always be in the list.

The `--gen-isas` / `--gen-dbt` flags are both on by default; pass one explicitly
only to skip the other. Encoding-translator pairs are built in
(`_ENCODING_TRANSLATOR_PAIRS` in `lib/python/amdisa/__main__.py`) — no extra flag
is needed. Hand-written files (`isa.h`, `insts.h`, `mma_exec.h`,
`addr_calc.h/.cpp`) are not overwritten by the generator.
