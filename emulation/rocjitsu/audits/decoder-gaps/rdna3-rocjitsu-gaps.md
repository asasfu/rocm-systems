...
- RDNA3 section 7.5.1 says inline constants used with packed math produce a
  ...
  sources produce an F16 constant value, in the cited manual passage.

Rocjitsu evidence:

- The generated ordinary packed-F16 VOP3P constructors use 32-bit `OPR_SRC`
  operands and replace literal extensions with 32-bit `OPR_SIMM32` operands in the implementation.
- `Operand::read_lane()` selects `resolve_src_scalar16()` only when the operand
  size is 16 bits; otherwise it uses `resolve_src_scalar()` in the implementation.
- The 32-bit inline resolver returns FP32 bit patterns such as `0x3F000000` for
  `0.5f` in the implementation, while the 16-bit resolver returns
  F16 encodings such as `0x3800` in the implementation.
- Shared packed-F16 execution reads raw 32-bit values and then selects 16-bit
  halves, for example `execute_v_pk_add_f16_vop3p` in the implementation and `execute_v_pk_fma_f16_vop3p`.
...
- The RDNA3 VOP3P packed-math field table defines `CLMP` as a result clamp:
  ...
  `[0, +max_uint]` in the cited manual passage.
...
- Ordinary packed-F16 helpers execute without testing `inst.inst_.clamp`, for
  ...
  `V_PK_MUL_F16` in the implementation.
...
- RDNA3 instruction definitions 32-34 describe `V_FMA_MIX_F32`,
  ...
  use `fma(...)` in pseudocode in the cited manual passage.
- The XML entries for `V_FMA_MIX_F32`, `V_FMA_MIXLO_F16`, and
  `V_FMA_MIXHI_F16` are present in the machine-readable ISA XML, and describe fused multiply-add behavior.

Rocjitsu evidence:

- `execute_v_fma_mix_f32_vop3p`, `execute_v_fma_mixhi_f16_vop3p`, and
  `execute_v_fma_mixlo_f16_vop3p` compute `float result = a * b + c;` in the implementation.
...
- The RDNA3 manual describes `DS_BVH_STACK_RTN_B32` as an LDS-backed per-thread
  ...
  invalidation in the cited manual passage.
- The RDNA3 XML lists opcode 173 with explicit `VDST`, in-out `ADDR`, `DATA0`,
  `DATA1`, and implicit DSMEM operands in the machine-readable ISA XML.
...
- The Python semantic classifier falls unrecognized DS instructions through to
  semantic class `nop` in the code generator, and the generator emits `UnimplementedInst` for `nop` in the code generator.
...
- The instruction is described as an LDS instruction for pushing/popping a
  per-thread stack in the cited manual passage.
- The XML entry includes implicit DSMEM input/output operands and places the
  instruction in the `VMEM`/`DATA_SHARE` functional group in the machine-readable ISA XML.

Rocjitsu evidence:

- The generated `DsBvhStackRtnB32Ds` constructor sets source/destination
  operands but does not set `flags_ |= MEMORY_OP` in the implementation.
- The generator only emits `MEMORY_OP` for recognized memory semantic classes,
  including DS read/write/atomic-style classes, in the code generator; this instruction currently falls through to
  `nop`.
...
- Chapter 1.1 defines GDS as scratch memory shared by all shader engines, with
  append-operation support, in the cited manual passage.
- Chapter 1.2.2.2 says RDNA3 devices use a 4KiB GDS visible to waves of a
  ...
  ordered append/consume support, in the cited manual passage.

Rocjitsu evidence:

- The KFD sysfs emulation advertises `gds_size_in_kb 0` for GPU nodes in the implementation.
- The device config struct has LDS, L1, and L2 fields but no GDS size field in the implementation.
- Generated RDNA3 DS execution bodies throw `UnimplementedInst` when the DS
  `gds` bit is set; for example `DsAddU32Ds` checks `inst_.gds` in the implementation.
...
Legal RDNA3 DS encodings that select GDS can decode, but they stop
...
- Chapter 2.1 says RDNA3 supports wave32 and wave64, and that shaders are
  compiled for one fixed wave size in the cited manual passage.
- The same section defines wave64 issue behavior, second-pass scalar
  input/output increments, and wave32 upper-`EXEC`/`VCC` ignored behavior in the cited manual passage.
...
- The W7900 RDNA3 config advertises `wave_front_size` 32 in the test configuration.
- `CommandProcessor::process_aql_packet` computes waves per workgroup from
  `cus_[0]->wf_size()` in the implementation; the AMDHSA `ENABLE_WAVEFRONT_SIZE32` kernel descriptor bit is
...
- The same section says single-wave workgroups do not count against the
  ...
  operations as `S_NOP` in the cited manual passage.

Rocjitsu evidence:

- The command processor derives `wg_size` from
  `workgroup_size_x * workgroup_size_y * workgroup_size_z` and computes
  `wfs_per_wg` in the implementation, then stores the dimensions directly in `DispatchEntry` in the implementation.
- `ComputeUnitCore::can_accept_workgroup()` admits workgroups based on free
  wave slots, register blocks, and LDS capacity in the implementation.
  It does not enforce a max-1024 work-item contract.
- The WGP allocation path increments `active_workgroups` and advances LDS
  allocation in the implementation, but searches found no 32-workgroup cap or single-wave barrier
  resource elision rule.
- `S_BARRIER` always transitions the wave to barrier state through
  `execute_s_barrier_sopp` in the implementation; barrier resolution waits for all wavefronts in the workgroup
...
- Chapter 2.2.2 describes pixel, geometry, and hull shader waves, says the
  ...
  launch modes in the cited manual passage.
...
- `CommandProcessor::init_wavefront_regs` initializes AMDHSA compute SGPRs and
  local work-item IDs in the implementation.
...
- Chapter 2.3 says WGP mode exposes one contiguous WGP LDS and allows waves in
  ...
  `LDS_DIRECT_LOAD` are not supported in WGP mode in the cited manual passage.

Rocjitsu evidence:

- RDNA3 generates concrete `LdsParamLoadLdsdir` and `LdsDirectLoadLdsdir`
  instruction classes in the implementation.
...
- The command processor records `dp.wgp_mode` for placement and LDS-capacity
  checks in the implementation, but the LDSDIR execute path has no WGP-mode rejection or
...
### RDNA3-RJ-011: RDNA3 HWREG state coverage remains incomplete

Manual/XML evidence:

- Chapter 3.1 and the RDNA3 XML expose MODE, STATUS, TRAPSTS, allocation,
  IB/PC/TBA/flat-scratch, identity, scheduler, and shader-cycle HWREG state.

Rocjitsu evidence:

- The shared RDNA3 descriptor table now uses MODE=1, STATUS=2, and the
  manual/XML IDs and names in the implementation; SOPK
  execution routes through the common field-access helpers. The former wrong
  map is patched.
- MODE and STATUS have backing state and policy. TRAPSTS, allocation, IB,
  PC/TBA, flat scratch, HW_ID, scheduler, performance, and shader cycles remain
  explicit unsupported descriptors.

Impact:

RDNA3 HWREG access no longer redirects IDs to unrelated state, but most of the
architectural inventory still cannot be read, written, or observed.
...
- Chapter 3.1 defines `EXECZ` and `VCCZ` as summary flags that indicate whether
  ...
  wave32 in the cited manual passage.
- Chapter 3.2.2 repeats that wave32 hardware acts only on `EXEC[31:0]`, and
  `EXECZ` reflects that low-half state in the cited manual passage.
...
- `Wavefront::set_exec()`, `set_exec_raw()`, and `set_vcc()` update only the
  live mask fields in the implementation.
- The scalar special-source resolver correctly synthesizes live `VCCZ` and
  `EXECZ` values from `wf.vcc()`/`wf.exec()` in the implementation, but raw `status_raw()` remains an independently stored field exposed
...
- Chapter 3.1 lists `TRAPSTS`, `TBA`, `TMA`, and `TTMP0`-`TTMP15` as
  shader-visible trap state in the cited manual passage.
- Chapter 3.2.1 says `S_RFE_B64` is one of the direct PC operations and that
  `S_TRAP` saves the PC of the trap instruction itself in the cited manual passage.
- XML exposes `HW_REG_TRAPSTS`, `HW_REG_SHADER_TBA_LO`,
  `HW_REG_SHADER_TBA_HI`, and the TMA/TBA return-message IDs in the machine-readable ISA XML.

Rocjitsu evidence:

- `Wavefront` stores status, mode, `EXEC`, `VCC`, `M0`, scratch/aperture bases,
  and wait counters, but has no `TRAPSTS`, `TBA`, or `TMA` state members in the implementation.
- RDNA3 `S_TRAP` now has no generated branch or program-terminator flag and
  dispatches to the empty shared `execute_s_trap_sopp()` helper in the implementation.
- Shared `execute_s_rfe_b64_sop1` is empty in the implementation, and the RDNA3 `S_RFE_B64` class dispatches to that helper in the implementation.
- RDNA3 `S_SENDMSG_RTN_B32` only returns a nonzero value for message `0x83` and
  returns zero for message IDs including `0x82`, `0x85`, and `0x86` in the implementation; the XML names those values
...
state. `S_TRAP` currently no-ops rather than entering the handler.
...
- Chapter 3.2.1 says the PC is a DWORD-aligned byte address whose low two bits
  are forced to zero in the cited manual passage.
- The same section says `S_SETPC_B64`, `S_RFE_B64`, and `S_SWAPPC_B64` transfer
  ...
  next instruction in the cited manual passage.

Rocjitsu evidence:

- `SSetpcB64Sop1::execute_impl` assigns
  `wf.pc = read_scalar64(ssrc0) - size_` without masking low address bits in the implementation.
- `SSwappcB64Sop1::execute_impl` likewise reads the raw scalar target, assigns
  it minus `size_`, and writes the next PC to the destination in the implementation.
- The outer CU loop special-cases `s_setpc` and `s_swappc`: if the raw scalar
  target is zero it halts the wave before executing the instruction in the implementation.
...
- Chapter 3.2.3 says hardware may skip vector instructions when `EXEC==0`,
  ...
  decisions that depend on outstanding VM/VScnt, EXPcnt, or LGKMcnt in the cited manual passage.
...
- Section 3.3.1.4 defines scalar selector regions, says out-of-range source
  ...
  says TTMP reads return zero and TTMP writes are ignored outside trap mode in the cited manual passage.
- The same section applies these rules to SALU, VALU, VMEM, and SMEM scalar
  operands, including SMEM destination-range suppression in the cited manual passage, and says `S_MOVREL` source OOR uses `S0` while destination OOR
  writes nothing in the cited manual passage.

Rocjitsu evidence:

- RDNA3 scalar source resolution reads SGPR and TTMP selector encodings directly
  from `wf.sgpr_alloc().base + ev` in the implementation, without checking `wf.num_sgprs()` or `STATUS.PRIV`.
- RDNA3 64-bit scalar source resolution reads `ev` and `ev + 1` directly for
  SGPR and TTMP ranges in the implementation.
- RDNA3 scalar writes and scalar64 writes write SGPR/TTMP selector encodings
  directly in the implementation, without privilege or
  out-of-range suppression.
- The underlying CU SGPR accessors index the physical SGPR file directly in the implementation.
...
- Section 3.3.1.3 says 64-bit SGPR data must be even-aligned, wider data must be
  ...
  also ignores the low bit of the index in the cited manual passage.
- Section 3.3.1.4 says multi-DWORD operands and GPR indexing must not cross
  ...
  TTMP range in the cited manual passage.

Rocjitsu evidence:

- RDNA3 `resolve_src_scalar64()` and `resolve_dst_write64()` use the encoded
  scalar selector as-is and access `ev + 1` for the high dword in the implementation; they do not clear low selector bits or
...
- RDNA3 `SMovrelsB32Sop1`/`SMovrelsB64Sop1` and
  ...
  `base + index * width_words` in the implementation.
- RDNA3 `SMovrelsd2B32Sop1` uses `M0[7:0]` for source and `M0[15:8]` for
  destination in the implementation, rather than enforcing the
...
- Section 3.3.1.2 says VCC is written by `V_CMP` and integer vector ADD/SUB,
  ...
  is a named SGPR pair subject to the same dependency checks as any other SGPR in the cited manual passage.

Rocjitsu evidence:

- `RegisterSet` defines `RegClass::VCC` but documents it as not currently
  tracked, and the set operations ignore untracked classes in the implementation.
- RDNA3 `Operand::to_register_ref()` maps only ordinary SGPR selector ranges for
  scalar operand types and drops `OPR_VCC` in the implementation.
- The E32 `V_CNDMASK_B32` and `V_ADD_CO_CI_U32` constructors list only explicit
  VGPR/SRC operands in the implementation, while their execution reads and/or writes VCC in the implementation.
- The VOP3 `V_DIV_FMAS_*` constructors list only `VDST` and `SRC0`-`SRC2` in the implementation, while
  execution reads fixed VCC in the implementation.
...
- Section 3.3.2.1 says VGPRs are allocated in blocks of 16 for wave32 or 8 for
  ...
  that waves cannot be created with zero VGPRs in the cited manual passage.
- The same section says a wave may deallocate all VGPRs via `S_SENDMSG`, cannot
  reallocate them afterward, and may only terminate after deallocation in the cited manual passage.

Rocjitsu evidence:

- The shared register allocator rejects zero-count allocations in the implementation, so
  zero-VGPR dispatch is already blocked.
- RDNA3 compute-unit setup uses the configured `vgprs_per_wf` as the physical
  per-slot block size in the implementation, and
  dispatch records the requested per-wave VGPR count directly in
  `wf->vgpr_alloc_.count` and `wf->num_vgprs_` in the implementation.
- The default config path sets RDNA3 `vgprs_per_wf` to 256 unless overridden in the implementation; no code found derives the physical allocation block
...
- `S_SENDMSG` decodes to an executable instruction, but the shared execution
  helper is a no-op in the implementation.
...
- Section 3.3.2.2 defines out-of-range VGPR operands from `Vs`/`Ve`, says
  ...
  VMEM, and export, with per-dword fallback for memory source groups in the cited manual passage.
- The same consequence table gives VOPD a different source rule: force the
  source address to `VGPRaddr % 4` in the cited manual passage.

Rocjitsu evidence:

- RDNA3 `Operand::read_lane()`, `write_lane()`, `read_lane64()`, and
  `write_lane64()` form `wf.vgpr_alloc().base + voff` and call physical VGPR
  accessors in the implementation; they do not compare `voff`/`voff + width - 1` against
  `wf.num_vgprs()` and do not substitute `VGPR0` or suppress writes.
- The SIMD operand chunk path similarly reads
  `wf.vgpr_alloc().base + voff` through a physical region in the implementation.
- VMEM address collection for buffer helpers reads physical regions starting
...
- Physical `RegisterAccess` and `RegisterFile` operations expose raw-index
  reads/writes and assertions, not architecture-level fallback/NOP behavior, in the implementation.
- RDNA3 VOPD execution reads its slot operands through normal `RegisterAccess`
  and writes both destinations through normal `write_lane()` in the implementation; no VOPD-specific `% 4` source fallback or
...
- Section 3.3.2.2 defines `V_MOVREL` indexed out-of-range checks, including
  `Index > 255` and `(Vs + M0)`/`(Ve + M0)` beyond `VGPR_SIZE`, in the cited manual passage.
- The consequence table says out-of-range destinations make the instruction a
  ...
  whole instruction if either destination is out of range in the cited manual passage.
- Chapter 7.8 separately lists the indexed VALU formulas, including unsigned
  ...
  `V_MOVRELSD_2_B32` and `V_SWAPREL_B32`, in the cited manual passage.
...
- RDNA3 `V_MOVRELD_B32` and `V_MOVRELS_B32` execute for in-range operands, but
  ...
  form a normal `OPR_VGPR` operand in the implementation; the VOP3 forms do the same in the implementation.
- RDNA3 `V_MOVRELSD_B32` and `V_MOVRELSD_2_B32` decode, but both E32 execute
  paths throw `UnimplementedInst` in the implementation; both VOP3 execute paths throw in the implementation.
- RDNA3 `V_SWAPREL_B32` marks both explicit operands as source and destination
  operands in the constructor in the implementation, but
  execution throws `UnimplementedInst`.
...
- Section 3.3.3 says memory/LDS/GDS reads and atomics with return must be
  ...
  `VDST+3` for `BUFFER_LOAD_B128`, in the cited manual passage.
- The same section says the destination check includes the extra PRT return
  ...
  is checked only for LDS/GDS/memory atomics that actually return a value in the cited manual passage.
...
- Generated RDNA3 `BUFFER_LOAD_B128` records a 128-bit `VDATA` destination and
  ...
  `d->num_elems = 4` before address calculation in the implementation; no pre-issue check nullifies the operation if `VDATA+3` is outside
  `wf.num_vgprs()`.
- Generated RDNA3 buffer atomics with return similarly set `d->dst_reg_base`
  from `inst_.vdata` and make return behavior depend on `GLC` in the implementation, while the DS atomic generator sets a
  returned `VDST` base for local-memory atomics in the code generator; neither path
...
- The common vector memory state stores `exec_mask`, `lane_mask`,
  ...
  PRT-return metadata in the implementation.
- Common vector memory completion only computes the number of destination VGPRs
  ...
  for an out-of-range destination window in the implementation.
- RDNA3 image instructions decode `DMASK`/`TFE` fields in machine structs and
  builders, but representative `image_load` execution is a minimal stub in the implementation,
...
- Section 3.3.3 says VMEM texture alignment is controlled by
  ...
  flat/scratch/global operations in the cited manual passage.
...
- RDNA3 scalar-memory address calculation does mask addresses down to a DWORD
  boundary in the implementation, but the VMEM/flat/DS helpers build per-lane addresses without
  consulting an alignment-mode register: buffer address calculation handles
  descriptor bounds in the implementation, flat/global/scratch address calculation directly forms
  addresses in the implementation, and DS address calculation directly adds the DS offset and
  LDS base in the implementation.
- GPU memory byte/dword/qword helpers perform unaligned `memcpy` at the current
  address when the translated page contains the bytes, in the implementation.
...
- Section 3.3.4 says LDS allocations are 0-64 KiB per wave/workgroup, shared by
  ...
  accesses are restricted to the allocated space in the cited manual passage.
...
- Dispatch validation and placement round `group_segment_fixed_size` to 256
  bytes, not 1024 bytes, in the implementation,
  the implementation,
  the implementation.
- WGP-mode uses a sibling-CU backing store whose size is the sum of the two CU
  LDS sizes in the implementation,
  and wavefronts carry only a placement-selected `lds_base`, not an allocated
  byte size, in the implementation.
- DS address calculation adds the DS byte offset and `wf.lds_base()` directly in the implementation; generated RDNA3 DS special forms do the same for scaled
  offsets in the implementation.
- LDS read/write helpers check only the total backing vector size, returning
  zero or dropping writes when the backing is crossed, in the implementation; the local-memory pipeline passes those backing-relative
  addresses straight through in the implementation.
...
- Section 3.3.4.1 says DS load/store sizes may be byte-aligned only when
  ...
  address bits, and 32-bit/64-bit atomics require 4-byte/8-byte alignment in the cited manual passage.
- The same section says LDS operations report `MEMVIOL` for out-of-range
  ...
  native LDS/GDS alignment masks for B8/B16/B32/B64/B96/B128 accesses in the cited manual passage.
...
- The shared DS address helper directly computes `addr + offset + wf.lds_base()`
  without an alignment mask in the implementation, and generated RDNA3 DS special forms similarly add scaled
  offsets without consulting alignment state in the implementation.
- LDS vector loads/stores and single-value helpers return zero or drop writes
  only when the total backing vector is crossed in the implementation; they do not report a violation or distinguish alignment
  modes.
...
- Section 3.4.1 says STATUS fields are readable but not normally writable by the
  ...
  bits are initialized at wave-creation time in the cited manual passage.
...
- `Wavefront::read_scc()` and `write_scc()` update only bit 0 in the implementation, and the
  raw STATUS accessors simply cast or assign the `StatusReg` in the implementation.
- `Wavefront::reset()` explicitly does not change the status register while
  clearing other dynamic dispatch state in the implementation,
  ...
  manual STATUS bits in the implementation.
...
- Section 3.4.2 says MODE `FP_ROUND` controls VALU floating-point rounding,
  ...
  `CLAMP`, and `IEEE` changes min/max NaN and signed-zero behavior in the cited manual passage.
- The same MODE table says `TRAP_AFTER_INST` forces trap-handler entry after
  ...
  preserving true infinities in the cited manual passage.

Rocjitsu evidence:

- `Wavefront` stores raw MODE and synchronizes only the VGPR-MSB subfield used
  by register indexing in the implementation.
- RDNA3 `S_ROUND_MODE` and `S_DENORM_MODE` decode and dispatch to shared helper
  functions, but those helpers are empty in the implementation.
- Representative RDNA3 floating-point helpers use host `std::fmax`/`std::fmin`
  ...
  `V_MAX_F32` in the implementation and `V_MIN_F32`.
...
- RDNA3 Chapter 3.1 lists `S0-S105` as scalar general-purpose registers and
  `FLAT_SCRATCH` as separate wave state in the cited manual passage.
- RDNA3 scalar and vector operand tables identify source values `0-105` as
  `SGPR0 - SGPR105`, while VOP3/VOP3P source values `209-232` are reserved, in the cited manual passage,
  the cited manual passage.
- RDNA3 XML likewise lists `s102` and `s103` as ordinary scalar predefined
  values in `OPR_SDST` and `OPR_SSRC` in the machine-readable ISA XML.
- CDNA3/CDNA4 are different: their manual operand tables name selector
  `102/103` as `FLAT_SCRATCH_LO/HI` in the cited manual passage, so the alias is not a
  cross-architecture rule.

Rocjitsu evidence:

- The generated RDNA3 operand bounds still classify scalar source and
  destination values through 105 as SGPRs in the implementation, and display/def-use code reports
  those values as SGPRs in the implementation.
- Execution special-cases source selector `102` and `103` before the ordinary
  SGPR range, returning `wf.scratch_base()` low/high halves in the implementation; the 64-bit source path returns the
  whole scratch base for selector `102`.
- The same resolver also accepts reserved source selector values `230` and
  `231` as scratch-base low/high halves in the implementation, even though the generated RDNA3 source enum
  has no flat-scratch source names in that range.
- Destination writes to selector `102` and `103` mutate `wf.scratch_base()`
  instead of SGPR102/SGPR103 in the implementation, and the
  64-bit destination path replaces the full scratch base for selector `102`.
...
- Section 3.4.7 says `FLAT_SCRATCH` is initialized by wave-launch hardware for
  ...
  read-only outside the trap handler, and is a 256-byte-aligned byte address in the cited manual passage.
- Section 3.4 and 3.4.8 also state that `FLAT_SCRATCH_LO/HI` are read-only
  HWREG targets writable only while in a trap handler in the cited manual passage.

Rocjitsu evidence:

- `Wavefront` stores a raw scratch base and exposes an unchecked
  `set_scratch_base()` setter in the implementation; reset
  zeroes `scratch_base_` and `scratch_lane_size_` in the implementation.
- Dispatch initializes scratch only when `private_segment_fixed_size > 0`, then
  ...
  stores it without any 256-byte alignment check or rounding in the implementation.
- Scratch and flat-private address helpers consume the stored base directly in the implementation.
...
- RDNA3 Section 3.4.10 says `SHADER_CYCLES` is a 20-bit graphics-core-clock
  ...
  for within-wave deltas, in the cited manual passage.
- The same section says `REALTIME` is a 64-bit fixed-frequency clock read with
  `S_SENDMSG_RTN_B64` followed by `S_WAITCNT LGKMcnt == 0` in the cited manual passage.
- XML exposes `HW_REG_SHADER_CYCLES` and `MSG_RTN_GET_REALTIME` in the machine-readable ISA XML.

Rocjitsu evidence:

- RDNA3 `SGetregB32Sopk::execute_impl` handles only HWREG IDs 1, 4, 5, 6, and
  7 before warning and returning zero for other IDs in the implementation; `HW_REG_SHADER_CYCLES` ID 29 therefore falls into the default path.
...
- `S_SENDMSG_RTN_B32` and `S_SENDMSG_RTN_B64` return `engine->global_time()` for
  message `0x83` and write the scalar destination immediately in the implementation; the shared generated template has the
  same synchronous write behavior in the implementation.
- `SimulationEngine::global_time()` is the latest processed simulation tick in the implementation. The
...
- Section 3.5.3.4 says compute launch appends a packed system SGPR containing
  ...
  `COMPUTE_PGM_RSRC2.tg_size_en` is set in the cited manual passage.

Rocjitsu evidence:

- The HSA descriptor header defines
  `COMPUTE_PGM_RSRC2_ENABLE_SGPR_WORKGROUP_INFO` in the implementation.
- `DispatchEntry` carries user-SGPR count, workgroup-id enable flags, and
  ...
  payload field in the implementation.
...
- `CommandProcessor::init_wavefront_regs()` writes AMDHSA user SGPRs, optional
  ...
  system SGPR write in the implementation.
...
- Section 3.5.3 says enabled SGPR launch values are packed consecutively with
  no alignment gaps in the cited manual passage.
- The compute table says `COMPUTE_PGM_RSRC2.tgid_x_en`,
  ...
  `work_group_id2[31:0]` respectively in the cited manual passage.
...
- `CommandProcessor::process_aql_packet()` keeps multidimensional dispatch
  ...
  `workgroup_id_x` in the implementation.
- `CommandProcessor::init_wavefront_regs()` computes `grid_wg_id_x` from
  ...
  grid_wg_id_x : global_wg_id;` in the implementation.
...
- Section 3.5.4 maps HS(+LS) and GS(+ES) combined-stage initial VGPR payloads
  ...
  ES parameter data, and GS Fast Launch variants in the cited manual passage.
- Section 3.5.4.1 says pixel shader VGPR loading uses a CAM from VS outputs to
  ...
  `ADDR` bits are set without corresponding `ENA` bits in the cited manual passage.
...
- `DispatchEntry` is compute/AQL shaped, with kernel dispatch IDs, grid sizes,
  ...
  launch payload fields or pixel-input register state in the implementation.
- `CommandProcessor::init_wavefront_regs()` initializes AMDHSA compute SGPRs,
  ...
  writes are local workitem IDs derived from the AQL packet in the implementation.
...
- Section 3.5.5 says only pixel shader waves have LDS pre-initialized before
  ...
  interpolated using barycentrics I/J to compute per-pixel parameters, in the cited manual passage.

Rocjitsu evidence:

- `DispatchEntry` is compute/AQL shaped, with kernel dispatch IDs, grid sizes,
  workgroup sizes, and workgroup ID controls, but no PS launch state, graphics
  parameter payload, or pixel-input LDS preload fields in the implementation.
- `CommandProcessor::dispatch_workgroups()` assigns an LDS base and backing
  store before wave register initialization in the implementation, and `process_aql_packet()` validates compute LDS capacity from
  `group_segment_fixed_size` in the implementation.
  Neither path populates LDS with pixel vertex parameter data before launch.
- `Lds` constructs a zeroed byte backing and exposes raw LDS read/write APIs in the implementation; it has no launch-time vertex-parameter preload
  helper.
- Static searches for `vertex parameter`, `barycentric`, `PS_INPUT`,
  ...
  `execute_lds_direct_load_ldsdir` are empty shared stubs in the implementation.
...
- Chapter 4.1 marks source selector values `209-232`, `239`, `249`, `251`,
  `252`, and `254` as reserved, while `250` is `DPP16`, in the cited manual passage.
...
- `can_resolve_src_scalar()` reports these values as resolvable by including
  `(ev >= 240 && ev <= 253)` and the non-RDNA3 `230`/`231` aliases in the implementation.
...
- Chapter 4.1 says a 32-bit literal used in a 64-bit signed integer operation
  ...
  operations zero-extend it, in the cited manual passage.

Rocjitsu evidence:

- `read_immediate64()` zero-extends every `OPR_SIMM32` value in the implementation, and both lane and scalar 64-bit immediate reads call it in the implementation.
- RDNA3 `V_MAD_I64_I32` has a signed 64-bit `SRC2` operand and literal
  encodings for that operand in XML in the machine-readable ISA XML.
- The generated constructor builds literal `SRC2` for `V_MAD_I64_I32` as
  `Operand(64, OperandType::OPR_SIMM32...)` in the implementation, and execution reads it with `read_lane64(inst.src2, lane)` in the implementation.
...
- Chapter 4.1.1 defines RDNA3 SLC/GLC/DLC cache behavior in the cited manual passage: GLC is the load-scope bit
...
- The implementation documents RDNA3/RDNA3.5 as `GLC`/`SLC` 2-bit
  scope plus `DLC` non-temporal, with `scope = (SLC << 1) | GLC`, in the implementation.
- `mtype_from_flags_gfx11()` forwards that scope and `DLC` to the CDNA-style
  `mtype_from_scope_nt()` helper in the implementation;
  the shared helper maps scope values `2+` to `Mtype::UC` and `nt` to
  `Mtype::NT` in the implementation.
- Generated RDNA3 memory instructions use this helper, for example SMEM
  `S_LOAD_B32` in the implementation, MUBUF `BUFFER_LOAD_U8` in the implementation, and FLAT `FLAT_LOAD_U8` in the implementation.
- The vector-memory pipeline passes only `d.mtype` and `d.non_temporal` into the
  vector L1 in the implementation, and the vector L1 bypasses/invalidate-paths on `Mtype::UC`,
  the separate `non_temporal` flag, or forced bypass in the implementation.
...
- The Chapter 4 introduction says unused fields that are not SGPR
  source/destination fields are typically set to zero in the cited manual passage.

Rocjitsu evidence:

- RDNA3 machine-instruction structs expose padding fields such as `pad_15_17`,
  `pad_53_56`, `pad_23`, and `pad_59_60` in the implementation.
...
- Section 5.1 says `S_CODE_END` is treated as an illegal instruction and is used
  to pad past the end of shaders in the cited manual passage.

Rocjitsu evidence:

- RDNA3 `SCodeEndSopp` decodes and dispatches through
  the implementation.
- The shared `execute_s_code_end_sopp` helper is empty in the implementation.
...
- Section 5.1 says `S_SETKILL` sets the kill bit and causes immediate program
  ...
  `USER_PRIO` and defines the priority formula in the cited manual passage.

Rocjitsu evidence:

- RDNA3 generated `S_SETKILL`, `S_SETHALT`, `S_SLEEP`, `S_WAKEUP`, and
  `S_SETPRIO` dispatch through the implementation.
- The shared helpers for `S_SETKILL`, `S_SETHALT`, `S_SETPRIO`, and `S_WAKEUP`
  are empty in the implementation.
- `execute_s_sleep_sopp` only requests a functional yield in the implementation; it does not track sleep duration or
...
- Section 5.1 defines `S_CLAUSE` length and break fields in the cited manual passage.
- Section 5.2 defines same-type uninterrupted clause execution, legal and
  ...
  clause-break causes in the cited manual passage.
...
- `ComputeUnitCore::step()` issues one instruction from each running wavefront
  in sequence in the implementation; no uninterrupted same-type clause arbitration or membership
...
- Section 5.1 says `S_SENDMSG` sends an upstream message without an enforced
  ...
  `S_SENDMSGHALT` sends a message and then halts in the cited manual passage.
...
- RDNA3 `S_SENDMSG` and `S_SENDMSGHALT` dispatch through
  the implementation, but the shared helpers are empty in the implementation.
- RDNA3 `S_SENDMSG_RTN_B32/B64` decode `OPR_SENDMSG` operands in the implementation, but execution only synthesizes a value for message `0x83` and
  otherwise writes zero in the implementation.
- The return-message path writes the scalar destination immediately and does not
  ...
  `LGKMCNT`/`KMCNT` support in the implementation.
...
- Section 5.6 says `S_WAIT_EVENT` waits for an event before proceeding,
  ...
  exceptions wait for this to complete before processing in the cited manual passage.

Rocjitsu evidence:

- RDNA3 `SWaitEventSopp` sets `flags_ |= WAITCNT` but its execute body only
  discards the wavefront reference in the implementation.
- The shared `execute_s_wait_event_sopp` helper is also empty in the implementation.
...
- Section 5.6 says FLAT instructions use both `LGKMcnt` and either `VMcnt` or
  `VScnt` in the cited manual passage.
- Chapter 11 repeats that FLAT instructions execute as both LDS and global
  ...
  done until both have decremented in the cited manual passage.

Rocjitsu evidence:

- `VectorMemState` stores a single `wait_counter_type` in the implementation, and
  `MemoryPipeline::issue()` increments only that selected counter in the implementation.
- Representative RDNA3 FLAT loads set only `WaitCounterType::LOADCNT` in the implementation,
  and representative FLAT stores set only `WaitCounterType::STORECNT` in the implementation.
...
- Chapter 6.7 says `S_AND_NOT{0,1}_WREXEC_B{32,64}` writes both `D` and `EXEC`,
  but `D cannot be EXEC` in the cited manual passage.
- XML encodes the explicit WREXEC destination as `OPR_SREG`, for example
  `S_AND_NOT0_WREXEC_B32` in the machine-readable ISA XML.
- `OPR_SREG` includes SGPRs, TTMPs, VCC, and `NULL`, but not `M0` or `EXEC`,
  in the machine-readable ISA XML; generated RDNA3
  `OpSelSreg` constants mirror that value set in the implementation.
...
- Chapter 6.5 says scalar conditional moves use SCC and do not write it in the cited manual passage.
...
- XML exposes these SCC dependencies structurally. For example, `S_ADD_I32`
  has an implicit SCC output in the machine-readable ISA XML,
  `S_CMOVK_I32` has an implicit SCC input, and
  `S_BITCMP0_B32` has an implicit SCC output.

Rocjitsu evidence:

- `DefUseChain` collects explicit operands and then calls `implicit_defs()` and
  `implicit_uses()` in the implementation.
- The base `Instruction` hooks are empty in the implementation.
- Representative SCC producer/consumer classes do not override those hooks:
  `SAddI32Sop2` is declared without SCC hooks in the implementation, and its constructor adds only explicit `SDST`, `SSRC0`, and `SSRC1`
  operands in the implementation.
- `SCselectB32Sop2` / `SCselectB64Sop2` add only explicit operands in the implementation, while execution reads SCC in the implementation.
- SOPC bit-compare classes similarly add only explicit source operands, for
  example `SBitcmp0B32Sopc` in the implementation.
...
- Chapter 7.1 defines VOP3 `ABS`, `OPSEL`, `CLAMP`, `OMOD`, and `NEG` fields in the cited manual passage.
- Chapter 7.5 defines VOP3P `NEG`, `NEG_HI`, `OPSEL`, `OPSEL_HI`, and `CLMP`
  fields for packed math in the cited manual passage.
- XML encodes these fields in the corresponding VOP3 and VOP3P formats, for
  example VOP3 literal fields in the machine-readable ISA XML.

Rocjitsu evidence:

- Generated RDNA3 raw instruction structs store VOP3 `abs`, `op_sel`, `clamp`,
  `omod`, and `neg` in the implementation, and VOP3P `neg_hi`, `op_sel`, `op_sel_hi_2`, `clamp`,
  `op_sel_hi`, and `neg`.
...
- The common disassembler prints explicit operands and then calls
  `build_modifiers()`, whose default implementation emits nothing, in the implementation.
...
- Chapter 7.1 restricts VOP3 `NEG`, `ABS`, and `OMOD` to floating-point use and
  says `OPSEL` must be zero for non-16-bit operands/results in the cited manual passage.
- Section 7.2.2.1 says input modifiers are undefined for non-FP inputs except
  ...
  readlane, writelane, permlane, and QSAD in the cited manual passage.
- Section 7.2.2.4 says VOP3 `OPSEL` is usable only by an allow-listed subset of
  VOP3 and promoted VOP1/VOP2/VOPC instructions in the cited manual passage.

Rocjitsu evidence:

- `Vop3MachineInst` preserves the raw VOP3 modifier and selector bits in the implementation, but the generated VOP3 base class only sizes literals and does
  not validate modifier legality in the implementation.
- Representative integer VOP3 forms such as `VAddNcU32Vop3` construct ordinary
  32-bit operands from the raw sources in the implementation; execution ignores `abs`, `op_sel`, `omod`, and `neg` while
  performing the add in the implementation. Legal integer `CLAMP` semantics are tracked separately in
  `RDNA3-RJ-053`.
...
- Chapter 7.1 says `VDST` used as a scalar destination for `V_READLANE` and
  ...
  `EXEC` while supporting `NULL`, in the cited manual passage.
- XML uses `OPR_SREG` for representative scalar destinations such as
  `V_READLANE_B32` `VDST` in the machine-readable ISA XML
  and `V_ADD_CO_CI_U32` `SDST` in the machine-readable ISA XML.

Rocjitsu evidence:

- `VReadlaneB32Vop3` builds scalar `vdst` directly from the raw VOP3 `VDST`
  field as `OPR_SREG` and writes through `RegisterAccess::write_scalar()` in the implementation.
- VOP3SD carry forms build `sdst` directly from the raw `SDST` field as
  `OPR_SREG`, for example `VAddCoCiU32Vop3SdstEnc` in the implementation; the shared carry helper writes that scalar destination in the implementation.
...
- Section 7.2.1 says readlane lane-select ignores upper bits and is limited to
  0-31 for wave32 or 0-63 for wave64 in the cited manual passage.
- XML identifies the lane-select source as `OPR_SSRC_LANESEL` for
  `V_READLANE_B32` and `V_WRITELANE_B32` in the machine-readable ISA XML.

Rocjitsu evidence:

- `VReadlaneB32Vop3::execute_impl()` reads the scalar lane selector and passes
  it unmodified to `RegisterAccess::read_lane()` in the implementation.
- `VWritelaneB32Vop3::execute_impl()` reads the scalar lane selector and passes
  it unmodified to `RegisterAccess::write_lane()` in the implementation.
- `Operand::read_lane()` and `Operand::write_lane()` forward the lane value to
  the VGPR accessors without applying a wave-size mask in the implementation.
...
- Section 7.2.2.3 limits VALU instructions to at most two scalar values, permits
  ...
  sharing restrictions in the cited manual passage.
- Section 7.2.5 and 7.2.6 identify mask/carry SGPR uses and distinguish
  broadcast SGPR data from per-lane mask bits in the cited manual passage.
...
- Representative VOP3 paths construct all raw sources directly with broad
  ...
  `OPR_SRC` operands in the implementation and
  `VAddCoCiU32Vop3SdstEnc` using two `OPR_SRC` values plus an `OPR_SREG` mask
  input in the implementation.
...
- Section 7.2.3.1 says `CLAMP` clamps integer results to the signed or unsigned
  ...
  `V_MUL_LO_U32`, in the cited manual passage.
- The XML generic VOP3 `CLAMP` field repeats the signed/unsigned integer clamp
  meanings and references a non-structured `OPF_NOCLAMP` list in the machine-readable ISA XML.

Rocjitsu evidence:

- Representative generated integer VOP3 constructors preserve the raw VOP3
  `clamp` field, such as `VAddNcU32Vop3` in the implementation.
- `execute_v_add_nc_i32_vop3` and `execute_v_add_nc_u32_vop3` use plain
  wraparound addition and never inspect `inst.inst_.clamp` in the implementation.
- `execute_v_sub_nc_i32_vop3`, `execute_v_sub_nc_u32_vop3`, and
  ...
  without `clamp` handling in the implementation.
- The integer SIMD helper documents that these VOP3 integer paths apply no
  result modifiers and that integer clamp would mean saturate, in the implementation.
...
- XML entries describe the standalone `V_PERMLANE16_B32`,
  ...
  adjacency hazard, in the machine-readable ISA XML.
...
- `VPermlane64B32Vop1` similarly dispatches directly to the shared permute
  executor in the implementation, and the shared helper
  only snapshots and swaps lanes in the implementation.
- The compute-unit execution path dispatches the current decoded instruction via
  ...
  `inst->execute(*inst, &wf)` directly in the implementation. The searched RDNA3 VALU
...
- Section 7.2.3.2 says wave64 VALU instructions may issue as two wave32 passes
  ...
  overwrite a scalar value needed by the second pass, in the cited manual passage.
- Section 7.2.5 and 7.2.6 identify VALU scalar mask/carry inputs and outputs in the cited manual passage.
...
- `VAddCoCiU32Vop3SdstEnc` binds `sdst` as a 64-bit `OPR_SREG` destination and
  `src2` as a 64-bit `OPR_SREG` carry input in the implementation, then the shared helper reads `src2` and writes `sdst` in the implementation.
- The generated VOP3/VOP3SD base constructors only size literal forms and do not
  validate scalar source/destination overlap in the implementation.
...
- Chapter 7.3 lists `V_PK_FMAC_F16`, `V_TRIG_PREOP_F64`,
  ...
  `V_QSAD_PK_U16_U8`, and `V_SAT_PK_U8_I16` in the non-VOP3P VALU inventory in the cited manual passage.
- The detailed opcode tables assign these instructions concrete VOP1, VOP2, or
  ...
  `V_SAT_PK_U8_I16`, `V_MULLIT_F32`, QSAD/MQSAD, and `V_TRIG_PREOP_F64`.
- The RDNA3 XML contains these affected instructions as concrete VALU entries,
  for example `V_SAT_PK_U8_I16` in the machine-readable ISA XML,
  `V_PK_FMAC_F16`, `V_MULLIT_F32`, QSAD/MQSAD, and `V_TRIG_PREOP_F64`.
...
- RDNA3 `packed_16bit_vgpr_dst` unconditionally treats any 16-bit `OPR_VGPR`
  destination value `128..255` as the high half of `v0..v127` in the implementation.
...
- Section 7.4 groups VINTERP with VOP3 and VOP3P as an OPSEL-based 16-bit
  encoding in the cited manual passage.
- Section 12.3.1 says 16-bit interpolation uses OPSEL to select the upper or
  lower half, gives real P10/P2 F16 formulas, and notes RTZ variants in the cited manual passage.
- Chapter 16.13 gives executable formulas for the F32 P10/P2 interpolation
  passes in the cited manual passage, and the F16/RTZ forms in the cited manual passage.
- The VINTERP format table carries `VDST`, `WAITEXP`, `OPSEL`, `CLMP`, source
  fields, and the F16/RTZ opcodes in the cited manual passage.
- XML lists all six VINTERP opcodes with interpolation descriptions and
  concrete operand widths in the machine-readable ISA XML.

Rocjitsu evidence:

- RDNA3 generates concrete constructors for the two F32 VINTERP forms in the implementation and for the four F16 VINTERP forms.
...
- Section 7.6 lists `DOT2ACC_F32_F16` and `DOT2ACC_F32_BF16` as VOPD `SRC2`
  users that read the destination operand in the cited manual passage.
- The VOPD OPX and OPY opcode tables list `V_DUAL_DOT2ACC_F32_F16` and
  `V_DUAL_DOT2ACC_F32_BF16` in the cited manual passage.
- Chapter 16.11 says both DOT2ACC forms accumulate with the destination and use
  the initial destination value as `S2` in the cited manual passage.

Rocjitsu evidence:

- RDNA3 `Vopd::op_name` names both DOT2ACC opcodes in the implementation, and decode smoke coverage includes DOT2ACC VOPD encodings in the relevant tests.
...
- `init_operands()` source collection special-cases `FMAC`, `MOV`, and
  ...
  not included in public source metadata in the implementation.

Impact:

Valid RDNA3 VOPD DOT2ACC instructions can decode and disassemble, but fail
...
- Section 15.3.7 gives finite OPX opcode values `0` through `13` and OPY
  opcode values `0` through `13`, `16`, `17`, and `18` in the cited manual passage.

Rocjitsu evidence:

- RDNA3 `Decoder::decode` routes any instruction whose top bits satisfy
  `Vopd::is_vopd()` to the VOPD constructor in the implementation.
...
- Section 7.6 says a VOPD pair must not use DPP in the cited manual passage.
- Section 15.3.7 says VOPD can be followed by a 32-bit literal constant, but
  not a DPP control DWORD, in the cited manual passage.
- The VOPD source selector table still assigns values `233`, `234`, and `250`
  to DPP selectors in the cited manual passage.

Rocjitsu evidence:

- RDNA3 VOPD routes non-literal source-0 selectors through generic `OPR_SRC` in the implementation.
- The RDNA3 `OPR_SRC` enum lists ordinary SGPR, inline, aperture, literal, SCC,
  and VGPR selector names but no DPP selector names in the implementation.
- Generic scalar source resolution treats selector `250` as zero/`NULL`, while
  ...
  exception rather than a VOPD-specific DPP legality diagnostic in the implementation.
...
- RDNA3 VOPD marks CNDMASK slots with `uses_vcc` in the implementation, and execution reads `wf.vcc()` for those slots in the implementation.
...
- Section 7.6 says `vsrc1X` and `vsrc1Y` are ignored for `V_MOV_B32` in the cited manual passage.
- The RDNA3 XML `V_DUAL_MOV_B32` entries expose only destination plus
  `SRC0`/literal operands, not `VSRC1`, in the machine-readable ISA XML.
...
- Source collection and disassembly correctly omit `src1` for MOV in the implementation, so execution-time behavior disagrees with the advertised source
  list.
...
- Section 7.6 says VOPD instruction pairs generate only a single exception if
  either or both paired operations raise an exception in the cited manual passage.

Rocjitsu evidence:

- `Vopd::execute_slot` directly performs each slot's arithmetic in the implementation.
...
- Section 8.1.2 says scalar buffer loads use the buffer resource descriptor's
  ...
  `(stride == 0 ? 1 : stride) * num_records` in the cited manual passage.
- Section 8.4 says `SBASE` for `S_BUFFER_LOAD` names a four-SGPR-aligned
  ...
  return zero, including partial multi-DWORD loads, in the cited manual passage.
...
- The generated `SBufferLoad*Smem` constructors expose `SBASE` as a 128-bit
  ...
  helper used by raw `S_LOAD_*` in the implementation.
- `smem_calculate_address()` reads only two scalar registers as a raw 64-bit
  base, adds the signed `OFFSET` plus `SOFFSET`, and clears low address bits in the implementation; it does not read descriptor stride or `num_records`, form the
...
- `ScalarMemPipeline` loads `num_dwords` directly from scalar L1 and writes
  every returned DWORD to SGPRs in the implementation; no per-DWORD scalar-buffer bounds mask or zero fill is visible.
...
- Section 8.1 defines `SOFFSET` as an SGPR, `M0`, or `NULL` unsigned byte
  offset in the cited manual passage.
- XML models `SOFFSET` as an explicit `OPR_SMEM_OFFSET` source operand on
  scalar loads and scalar buffer loads in the machine-readable ISA XML, with
  `OPR_SMEM_OFFSET` partitioned into `M0`, `NULL`, and scalar-register
  subtypes in the machine-readable ISA XML.
...
- RDNA3 generated SMEM constructors build the public `soffset` operand with
  ...
  enc->offset)` in the implementation.
- The constructors then publish that operand as `src_operands_[1]` for both
  raw and buffer SMEM loads, for example in the implementation.
- Execution still reads the raw instruction `soffset` field through
  `read_smem_offset()`, including `NULL`, `M0`, and SGPR cases, in the implementation.
...
- Section 8.2 says `LGKMcnt` increments by 1 for single-DWORD fetches or cache
  ...
  same amount on completion in the cited manual passage.
- The same section says scalar cache invalidates are not known complete until
  the shader waits for `LGKMcnt == 0` in the cited manual passage.
...
- `MemoryPipeline::issue()` increments exactly once for the selected counter
  and decrements exactly once on completion in the implementation;
  `WaitCounters::increment(KMCNT)` increments `KMCNT` and `LGKMCNT` by one in the implementation.
- `S_GL1_INV` and `S_DCACHE_INV` have no `MEMORY_OP` flag or scalar-memory
  state and execute direct cache invalidations in the implementation.
...
- Section 8.3 says scalar memory instructions issue in groups and that `INV`
  instructions must be in a group by themselves and may not be in a clause in the cited manual passage.

Rocjitsu evidence:

- `S_GL1_INV` and `S_DCACHE_INV` decode as ordinary operandless SMEM
  instructions and execute direct cache invalidation helpers in the implementation.
...
- Section 9.4 defines buffer address calculation from descriptor base, SGPR
  ...
  add-tid, and optional swizzle fields in the cited manual passage.
- Section 9.4.1 defines the two-bit `OOB_SELECT` modes, payload-inclusive
  ...
  MUBUF versus MTBUF B64/B96/B128 range behavior in the cited manual passage.
- Sections 9.4.1.1 through 9.4.2 give different structured, raw, scratch,
  scalar, and swizzled address formulas in the cited manual passage.

Rocjitsu evidence:

- `mubuf_calculate_addresses()` reads descriptor base, stride, `num_records`,
  and one `oob_raw` bit from `srd3 >> 31`, then always forms
  `index * stride + offset + soffset` before a lane-level OOB check in the implementation.
- The MTBUF helper duplicates the same model in the implementation.
- `VectorMemState` carries one `lane_mask` and a single
  `elem_size`/`num_elems` tuple in the implementation, and
...
- Section 9.2 says formatted buffer loads/stores return float, unsigned, or
  ...
  components into VGPR halves in the cited manual passage.
- Section 9.3 says MTBUF data format comes from the instruction, MUBUF
  ...
  and format/opcode component-count mismatches have defined behavior in the cited manual passage.
- Chapter 16.16 and 16.17 define MUBUF and MTBUF formatted operations in terms
  ...
  packing, in the cited manual passage.
...
- RDNA3 MUBUF formatted loads/stores, including D16 formatted variants, throw
  ...
  `BUFFER_STORE_FORMAT_XYZW` in the implementation, D16 formatted forms, and D16_HI format-X
  forms.
- RDNA3 MTBUF non-D16 formatted instructions execute as fixed 32-bit
  ...
  example in the implementation, while D16 MTBUF formatted variants throw.
- The shared vector memory state has no resource-format or `DST_SEL` fields in the implementation.
...
- The address helpers do not call the operand resolver. They special-case only
  ...
  `read_sgpr(wf.sgpr_alloc().base + inst.soffset)` in the implementation for MUBUF for MTBUF.
- The scalar resolver already knows how to return zero for `NULL`, `wf.m0()`
  for `M0`, and inline constant values for ordinary scalar sources in the implementation, but that path is bypassed here.
...
- Chapter 16.16 defines `BUFFER_ATOMIC_CSUB_U32` as subtracting the data value
  from memory and clamping the stored result to zero if `old_value < DATA` in the cited manual passage.
...
- Section 9.1 says `TFE` enables partially resident texture fault reporting for
  ...
  fetch-destination VGPR on a NACK in the cited manual passage.

Rocjitsu evidence:

- RDNA3 machine instruction structs store `tfe` for MUBUF and MTBUF in the implementation.
...
- `VectorMemState` tracks destination base, element size, lane mask, D16 flags,
  ...
  metadata in the implementation.
...
- The Chapter 9 MUBUF table says `BUFFER_GL0_INV` invalidates the shader L0
  ...
  the GL1 cache associated with the wave's VMID in the cited manual passage.

Rocjitsu evidence:

- `execute_buffer_gl0_inv_mubuf()` and `execute_buffer_gl1_inv_mubuf()` both
  invalidate the CU vector L1 and flush all L2 state for the process in the implementation.
...
- Generated image atomic, query, sample, gather, and G16 sample bodies also do
  nothing, for example atomic/query bodies in the implementation and
  sample/gather bodies in the implementation.
- The code generator explicitly emits these image-pipeline stubs for
  ...
  `image_bvh` in the code generator.
...
- Section 10.1 says `DMASK` selects the number and order of returned or stored
  ...
  and A16 change packed data/address component handling in the cited manual passage.
- Sections 10.1.5 through 10.4 define NSA grouping and per-op/per-DIM address and
  data VGPR usage in the cited manual passage.
...
- Generated MIMG constructors bind fixed-width operands from the mnemonic class,
  such as `IMAGE_LOAD` with 128-bit `VDATA` and 128-bit `VADDR` in the implementation,
  `IMAGE_GET_RESINFO` with 128-bit `VDATA` and 32-bit `VADDR`, and sampler forms with fixed `VADDR` widths.
...
- Section 10.9 and Chapter 16.18 define `IMAGE_BVH_INTERSECT_RAY` and
  ...
  and encoding restrictions in the cited manual passage.

Rocjitsu evidence:

- `ImageBvhIntersectRayMimg::execute_impl()` calls
  `amdgpu::execute_image_bvh_intersect_ray_mimg()` in the implementation,
  but that shared helper is an empty inline function in the implementation.
...
- Chapter 11 lists `GLOBAL_ATOMIC_CSUB_U32` as the GLOBAL-only opcode 55 and says
  `GLC` must be set to 1 in the cited manual passage; Chapter 15.9 lists it in the
  GLOBAL opcode table in the cited manual passage.
- Chapter 16.20 defines the clamped subtract operation and return value in the cited manual passage.
...
- Searches of the generated RDNA3 FLAT/GLOBAL surface found no
  ...
  the only RDNA3 `Csub` matches are for `BUFFER_ATOMIC_CSUB_U32` in the implementation,
  the implementation, and the decode fixtures.
- The shared RDNA3 `sub_decode_flat` table implements opcode slots 51-54 and
  56-64, but slot 55 is `decodeInvalid` in the implementation.
- The generated RDNA3 flat encoding fixture jumps from
  `flat_atomic_sub_u32` to `flat_atomic_min_i32`, with no
  `global_atomic_csub_u32` or `flat_atomic_csub_u32`, in the decode fixtures.
...
- Chapter 11 says flat addresses are tested per work-item against memory aperture
  ...
  global, private scratch, shared LDS, and invalid space in the cited manual passage.
- The field and format tables say FLAT uses a 12-bit unsigned offset whose MSB is
  ignored/forced zero, while GLOBAL/SCRATCH use signed 13-bit offsets, in the cited manual passage.
- Section 11.2 gives the SEG-specific global, scratch, and flat address formulas,
  including flat LDS/shared address calculation and the SADDR/SVE mode table, in the cited manual passage; Section 11.3 says invalid-aperture
  checks occur before offsets and other error checks after offsets in the cited manual passage.

Rocjitsu evidence:

- RDNA3 `flat_calculate_addresses()` sign-extends `inst.offset` as 13 bits for all
  SEG values in the implementation, so flat offsets with bit 12 set subtract from the address instead of
  ignoring that bit.
- The helper forms the final address as `saddr_val + vaddr + offset` before
  ...
  32 bits against `private_aperture_base() >> 32`, in the implementation.
- Shared-aperture routing is performed later in `ComputeUnitCore::route_memory_inst`
  ...
  issuing it to LDS in the implementation.
...
- Section 11.2 defines scratch addressing as
  ...
  scratch swizzle formula using wave32 or wave64 stride in the cited manual passage.
- The flat-private address formula also maps flat private addresses through
  `FLAT_SCRATCH` plus the scratch swizzle in the cited manual passage.

Rocjitsu evidence:

- Direct scratch SEG handling computes addresses as
  `scratch_base + lane * scratch_lane_size + vaddr + saddr_val + offset` in the implementation.
- Flat private-aperture handling uses the same contiguous lane-slice model after
  the aperture check in the implementation.
- Dispatch scratch setup allocates a per-wave region of
  `private_segment_fixed_size * wf_size`, stores `wave_scratch` as the wave
  scratch base, and records `private_segment_fixed_size` as `scratch_lane_size` in the implementation.
...
- The Chapter 11 field table says `ADDR` is a 64-bit address for FLAT and
  ...
  offset only if `SVE=1` in the cited manual passage.
- Chapter 15.9 says `SADDR` is disabled by `NULL` or `0x7f`, and repeats that
  Scratch uses `SVE` to decide whether a VGPR offset participates, in the cited manual passage; Section 11.2 lists the ST/SS/SV/SVS
  modes in the cited manual passage.

Rocjitsu evidence:

- Runtime address calculation treats both `0x7c` and `0x7f` as disabled SADDR
  encodings in the implementation.
- The generator's flat constructor patch-up checks only `inst_.saddr != 0x7F`
  ...
  `addr` to a 32-bit VGPR operand without considering `SVE`, in the code generator; generated
  `FlatLoadU8Flat` shows the same pattern in the implementation.
- `Flat::implicit_uses()` likewise skips only `0x7f`, not RDNA's `NULL` selector
  value 124, before adding a scratch/global SADDR register use in the implementation.
...
- Chapter 12 says the `M0` register is used by `ds_load_addtid_b32` and
  ...
  LDS-indexed operations, in the cited manual passage.
- The ADDTID address formula is
  `LDS_BASE + {InstOffset1, InstOffset0} + TID(0..63)*4 + M0`, with no VGPR
  address contribution and DWORD-aligned `M0`, in the cited manual passage.

Rocjitsu evidence:

- `DS_STORE_ADDTID_B32` has no explicit `ADDR` operand, but its execution path
  calls the generic `ds_calculate_addresses()` helper in the implementation.
- The shared DS helper always reads the encoded `inst.addr` VGPR and computes
  `VGPR[ADDR] + {offset1,offset0} + lds_base`, in the implementation. It does not apply `M0` or `TID*4`.
- `DS_LOAD_ADDTID_B32` uses a custom helper, but that helper interprets
  `M0[24:16]` as a per-lane stride and computes
  `lane * ds_stride_bytes + offset + lds_base` in the implementation. It ignores the manual's `M0[15:0]` byte offset and hardcoded
  `TID*4` term.
...
- Chapter 12 says LDS indexed and atomic instructions use `LGKMcnt`, increment
  ...
  other LDS instructions from the same wave in the cited manual passage.
- The same chapter lists `DS_PERMUTE_B32` and `DS_BPERMUTE_B32` in the LDS
  ...
  memory storage, and may run without LDS allocation in the cited manual passage.
- Chapter 16.15 defines `DS_SWIZZLE_B32` as a DS dword swizzle that does not
  read or write DS memory banks and returns zero for invalid source threads in the cited manual passage.
...
- Generated ordinary DS memory operations allocate `VectorMemState`, set
  ...
  pipeline. `DS_STORE_ADDTID_B32` is one example in the implementation.
- `DS_SWIZZLE_B32`, `DS_PERMUTE_B32`, and `DS_BPERMUTE_B32` constructors do not
  set `MEMORY_OP`; they call immediate shared helpers directly in the implementation.
- Those shared helpers read and write VGPR regions directly and never create a
  `VectorMemState` or set `DSCNT`/`LGKMcnt` participation in the implementation.
...
- Chapter 16.15 defines `DS_WRAP_RTN_B32` as opcode 52 for ring-buffer wrap
  ...
  the original value in the cited manual passage.
- The RDNA3 XML has a `DS_WRAP_RTN_B32` entry with `VDST`, `ADDR`, `DATA0`,
  `DATA1`, implicit DSMEM input/output operands, and
  `VMEM`/`ATOMIC`/`DATA_SHARE` classification in the machine-readable ISA XML.
...
- Chapter 13 says float-atomic-add ignores `MODE.round` and is fixed to
  round-to-nearest-even, in the cited manual passage.
- Section 13.2 gives different denormal behavior for LDS add, LDS
  ...
  memory atomic add always flushes input denormals, in the cited manual passage.
- Section 13.3 defines float-atomic SNaN quieting, min/max ordering across NaN,
  ...
  equality with `+0 == -0`, and float-add NaN/infinity/zero special cases in the cited manual passage.

Rocjitsu evidence:

- Generated DS F32 atomics set `AtomicOp::CMPSWAP`, `FMIN`, `FMAX`, and `FADD`
  before entering the shared memory path in the implementation,
  and DS F64 compare/min/max forms use the same `CMPSWAP`/`FMIN`/`FMAX`
  operation IDs in the implementation.
- Generated FLAT/GLOBAL/SCRATCH F32 and buffer F32 atomics use the same
  `CMPSWAP`, `FMIN`, `FMAX`, and `FADD` operation IDs in the implementation.
- The shared memory pipeline treats only `FADD`/`FMIN`/`FMAX` as FP operations
  and implements them with host `+`, `std::fmin`, and `std::fmax` in the implementation. It does not apply the Chapter 13 denormal table,
...
- Section 13.4 says GWS instructions use `LGKMcnt`, must be issued as a
  ...
  NACK/replay restrictions on source and destination VGPR aliasing in the cited manual passage.
- Section 13.4.2 says GDS/GWS instructions are single-lane, have special
  ...
  LGKMcnt-visible traffic for other `EXEC==0` GDS/GWS opcodes in the cited manual passage.
- Sections 13.4.3 and 13.4.4 define ordered-count queueing, field
  ...
  semaphore/barrier pseudocode in the cited manual passage.
...
- `DS_ORDERED_COUNT` similarly has a generated constructor but its execution
  body throws unconditionally in the implementation.
...
- Chapter 14 says exports copy enabled `EXEC` lanes from VGPRs to graphics
  ...
  `ROW_EN` uses `M0` for mesh position/primitive rows in the cited manual passage.
- Section 14.1 defines pixel export obligations, depth/MRT0/sample-mask
  ordering, and dual-source blend adjacency plus lane-mask transforms in the cited manual passage.
- Section 14.3 says export issue increments `EXPCNT`, completion later reads
  ...
  makes exports act as NOPs in the cited manual passage.
- Chapter 16.19 says pixel shaders must export to a color, depth, or `NULL`
  ...
  exports, and the final position export must set `DONE` in the cited manual passage.

Rocjitsu evidence:

- Generated RDNA3 `ExpExp` exposes `TGT` and four VGPR sources, but does not add
  the XML/manual implicit `EXEC` or `M0` operands to its source list in the implementation.
- `ExpExp::execute_impl()` ignores the wavefront and returns as a compute
  simulation no-op in the implementation.
- The RDNA3 `Exp` encoding base does not override implicit-use handling or
  install export-specific state in the implementation.
...
- Chapter 16.2 defines `S_WAITCNT_VSCNT`, `S_WAITCNT_VMCNT`, and
  ...
  comparison and no clamping on add overflow, in the cited manual passage.
- The same section defines `S_WAITCNT_EXPCNT` as
  `S0.u[2:0] + S1.u[2:0]`, with a three-bit comparison and no clamping, in the cited manual passage.
- Literal-only waits are encoded by writing `NULL` for the GPR argument, so a
  non-`NULL` SGPR operand is architecturally meaningful in the cited manual passage.

Rocjitsu evidence:

- The RDNA3 split wait constructors keep both the `SDST`/`OPR_SDST_NULL`
  operand and the `SIMM16` operand in `src_operands_` in the implementation.
- Their execute bodies read only `simm16.encoding_value_` and set the wait
  target from that immediate value in the implementation. No
...
- Chapter 3.4.1 defines `STATUS.CDBG_USER` and `STATUS.CDBG_SYS` as
  conditional-debug bits that can be used in conditional branches in the cited manual passage.
- Chapter 16.5 defines `S_CBRANCH_CDBGSYS`, `S_CBRANCH_CDBGUSER`,
  ...
  conditional branches on those status bits in the cited manual passage.

Rocjitsu evidence:

- The four RDNA3 debug-branch constructors dispatch to shared helpers in the implementation, but unlike the SCC/VCC/EXEC branch constructors they do not set
  `flags_ |= COND_BRANCH`.
- The shared helpers for `execute_s_cbranch_cdbgsys_sopp`,
  ...
  `execute_s_cbranch_cdbgsys_and_user_sopp` are empty in the implementation.
...
- Chapter 16.8 defines compact `V_PIPEFLUSH` opcode 27 as flushing the VALU
  destination cache in the cited manual passage.
...
- XML carries the operandless `V_PIPEFLUSH` instruction and describes flushing
  the vector ALU pipeline through the destination cache in the machine-readable ISA XML.
...
- The detailed `V_CMPX_LT_F16` definition stores to `EXEC.u64[laneId]` and says
  to write only `EXEC` with `SDST` set to `EXEC_LO` in the cited manual passage.
- XML carries the VOP3 alias result as an `OPR_SREG`/`VDST` output for
  `V_CMP_LT_F16` in the machine-readable ISA XML, and the
  `CMPX` result as `OPR_EXEC` plus implicit `OPR_SDST_EXEC` for
  `V_CMPX_LT_F16` in the machine-readable ISA XML.
...
- Generated compact `VCmpxFF16Vopc` likewise publishes only `src0` and `vsrc1`
  and sets `num_dst_ = 0` in the implementation.
...
- Execution still mutates hidden masks: shared non-`CMPX` helpers write
  ...
  execute bodies write `EXEC`, for example the implementation.
...
- Chapter 7.7 says DPP may be used with VOP3, excluding the listed 64-bit and
  special opcodes, while VOPC 64-bit opcodes are excluded separately in the cited manual passage.
- The same DPP section says `V_CMP` and `V_CMPX` write the full mask, not a
  ...
  and `FI=1` still does not turn on inactive lanes for `V_CMPX` in the cited manual passage.
- DPP16 row and bank masks say that for VOPC, disabled-lane SGPR/VCC bits
  receive zero in the cited manual passage.
- Chapter 16.12 says all non-`VOP3SD` VALU instructions use VOP3 in the cited manual passage; representative `CMPX` definitions
  write only `EXEC` with `SDST` set to `EXEC_LO`, for example
  `V_CMPX_T_U32` in the cited manual passage.
- XML carries VOP3 compare-result operands as scalar destinations, for example
  ...
  `V_CMPX_F_U64` in the machine-readable ISA XML, so the
...
- Generated RDNA3 VOP3 compare constructors accept DPP8 and DPP16 source-0
  marker forms; for example `VCmpLtF16Vop3` loads the DPP suffix state in the implementation.
- `VCmpLtF16Vop3::execute_impl` computes a full scalar compare mask across
  ...
  write mask in the implementation; the later mask path
  restores a VGPR destination, not scalar
  compare-result bits.
- `VCmpxFU64Vop3::execute_impl` similarly accepts DPP suffix state, calls
  ...
  indexed by `VDST` in the implementation.
...
- Chapter 7.7 defines `BOUND_CTRL` as the selector between writing zero and
  disabling the write when a DPP source lane is invalid or out of range in the cited manual passage; `FI=0` says inactive source lanes use `bound_ctrl`,
  while `FI=1` fetches inactive lanes but still uses `bound_ctrl` for
  out-of-range lanes in the cited manual passage.
- The BC/FI table explicitly says `BC=0/FI=0` disables the destination write
  for both out-of-range source lanes and in-range but disabled source lanes in the cited manual passage.
- XML's DPP16 field records also describe `BOUND_CTRL=0` as clearing
  ...
  lanes in the machine-readable ISA XML.

Rocjitsu evidence:

- The shared DPP16 destination write mask only accounts for row/bank masks and
  out-of-range permutations; it has no `EXEC`/inactive-source input in the implementation.
- `dpp_read()` receives `exec_mask`, but for `FI=0` it returns zero for an
  inactive source lane regardless of `bound_ctrl` in the implementation; `apply_dpp()` passes the current
  `EXEC` mask into this helper.
- Generated DPP users restore the old VGPR destination only for lanes excluded
  ...
  lanes not present in `dpp_write_mask()` in the implementation.
...
- Chapter 7.7 says `V_CMP`/`V_CMPX` write a full mask under DPP, and with
  ...
  still does not turn on inactive lanes for `V_CMPX` in the cited manual passage.
- The DPP16 row and bank mask descriptions say that for VOPC, disabled
  SGPR/VCC bits receive zero in the cited manual passage.
- XML's VOPC DPP16 encoding description says compact compares respect `EXEC`
  ...
  records row/bank/bound/FI fields in the machine-readable ISA XML.
...
- `VCmpFF16Vopc::execute_impl` snapshots old `VCC`, computes a DPP write mask,
  ...
  value rather than forcing them to zero in the implementation.
- `VCmpxFF16Vopc::execute_impl` does the same for both `VCC` and `EXEC`:
  ...
  bits back from the old values in the implementation.
- Shared compact VOPC compare helpers do produce a fresh zero-initialized mask
  ...
  `execute_v_cmp_lt_f16_vopc` in the implementation; the generated DPP merge overwrites that full-mask behavior
...
- Chapter 7.9 says WMMA is encoded with VOP3P, lists only the six matrix
  instructions, and says inline constants can only be used for the C matrix in the cited manual passage.
- Chapter 7.7's DPP support table explicitly lists WMMA under the VOP3P
  "NO DPP" set in the cited manual passage.
- XML records the WMMA entries with VGPR-only A/B operands and
  ...
  DPP8, or DPP16 encodings, in the machine-readable ISA XML.
...
- The generated RDNA3 WMMA constructors all rewrite `SRC0 == 255`,
  ...
  ranges are `VWmmaF3216x16x16F16Vop3p` in the implementation, `VWmmaF3216x16x16Bf16Vop3p`,
  `VWmmaF1616x16x16F16Vop3p`,
  `VWmmaBf1616x16x16Bf16Vop3p`,
  `VWmmaI3216x16x16Iu8Vop3p`, and
  `VWmmaI3216x16x16Iu4Vop3p`.
- The same constructors accept DPP8 and DPP16 source-0 selector forms and store
  DPP suffix state, for example in the implementation.
- The execute paths then compute WMMA source bases from `src0.encoding_value_`
  and `src1.encoding_value_` directly, for example in the implementation, so illegal literal or DPP forms
...
- Chapter 7.9 says F16/BF16 WMMA repurposes `NEG[1:0]` for SRC1/SRC0 low-half
  ...
  `{NEG_HI[2], NEG[2]}` for SRC2 `{ABS, NEG}` in the cited manual passage.
- The same paragraph says integer IU8/IU4 WMMA uses only `NEG[1:0]` for A/B
  signedness and requires `NEG[2]` plus `NEG_HI[2:0]` to be zero in the cited manual passage.
...
- The F32-output F16/BF16 WMMA execute paths pass plain `extract_f16` or
  ...
  `wmma_c_modifier(inst_.neg, inst_.neg_hi)`, in the implementation.
- The F16/BF16-output WMMA execute paths also pass plain A/B extractors, but do
  ...
  `exec_gfx11_wmma_bf16`, in the implementation.
- The shared packed16 WMMA helper has no C-modifier parameter and initializes a
  floating accumulator directly from `const_acc` or `read_acc`, in the implementation. The only shared C-modifier helper applies `{ABS, NEG}` to C in the
  F32-output path in the implementation.
...
- Chapter 7.9 says back-to-back dependent WMMA instructions require one
  ...
  A/B matrices, in the cited manual passage.
- XML lists the standalone WMMA instruction entries and operand shapes in the machine-readable ISA XML, but does not encode this
  instruction-stream adjacency rule.

Rocjitsu evidence:

- The compute-unit path dispatches each decoded instruction directly through
  `execute_instruction(inst, *active)` in the implementation,
  and the RDNA3 compute-unit override calls the instruction's execute callback
  without stream-history validation in the implementation.
...
- Chapter 16.10 defines `V_DOT2_F32_BF16` as a dot product of packed BF16
  values converted to F32 before multiplication and accumulation in the cited manual passage.
...
- The RDNA3 VOP3P field table defines `CLMP` as a result clamp: signed integer
  ...
  clamps to `[0, +max_uint]`, in the cited manual passage.
- Chapter 16.10 defines `V_DOT4_I32_IU8` and `V_DOT8_I32_IU4` as signed-domain
  ...
  interpretation for SRC0/SRC1, in the cited manual passage.
...
- `execute_v_dot4_i32_iu8_vop3p` and `execute_v_dot8_i32_iu4_vop3p` use
  ...
  any negative result to zero in the implementation.
- `execute_v_dot4_u32_u8_vop3p` and `execute_v_dot8_u32_u4_vop3p` perform
  ...
  inspecting `inst.inst_.clamp` in the implementation.
...
- Section 15.10.1 defines `EXP` fields for `EN`, `TARGET`, `DONE`, `ROW`, and
  `VSRC0` through `VSRC3` in the cited manual passage.
- Chapter 16.19 says pixel shaders must use a color, depth, or `NULL` target
  ...
  obligations in the cited manual passage.
- XML `ENC_EXP` records `EN`, `DONE`, `ROW_EN`, `TGT`, and `VSRC0` through
  `VSRC3`, and its prose shows disabled channels as `off`, in the machine-readable ISA XML.

Rocjitsu evidence:

- `ExpMachineInst` stores `en`, `done`, and `row_en`, but bit 12 is generated as
  `pad_12` rather than a `vm` field in the implementation.
- `ExpBuilderFields` and `build_exp()` expose `en`, `done`, and `row_en`, but
  no `vm` field in the implementation.
- `ExpExp` always constructs four `OPR_VGPR` sources and one `OPR_TGT`
  destination, regardless of `EN`, in the implementation.
- Generic `Instruction::disassemble()` prints only destination operands, source
  operands, and `build_modifiers()` output in the implementation, while
  RDNA3 `Exp` does not override `build_modifiers()` in the implementation.
- The generated `OPR_TGT` enum and formatter name MRT0-7, MRTZ, POS0-4, PRIM,
  and dual-source blend targets, but not a `NULL` target in the implementation.
...
- Chapter 15.1 scalar/control decode narrow match: RDNA3 imports shared
  field-identical SOP1/SOPC/SOPP/SOPK/SOP2 structs in the implementation, and those shared structs preserve the Chapter 15 bit positions in the implementation. The generated encoding
  classes size ordinary and literal scalar encodings in the implementation, including the SOPK implied-literal case for
  `S_SETREG_IMM32_B32`.
...
- Chapter 16.4 SOPC execution narrow match: generated SOPC compare and
  ...
  indexes to five or six bits in the implementation. The remaining SCC concern is the
...
- Chapter 16.10 ordinary VOP3P packed-operation boundary: the generated RDNA3
  ...
  `V_PK_ADD_I16`, `V_PK_LSHLREV_B16`, and `V_PK_MAD_I16` in the implementation. Existing gaps cover inline constants
...
- Chapter 15.3.1-15.3.5 VALU decode narrow match: generated RDNA3 machine
  ...
  layouts in the implementation; generated builders also expose the VOP3SD caller-controlled fields in the implementation. Existing VALU gaps are about execution
...
- Chapter 15.3.8-15.3.9 DPP extension narrow match: the generated VOP1, VOPC,
  ...
  operand slots in the implementation; the generated constructors decode DPP lane selectors and DPP control
  fields before applying shared DPP helpers, for example in
  the implementation. Remaining DPP runtime issues are tracked
  under `RDNA3-RJ-093` through `RDNA3-RJ-095`.
- Chapter 15.4 VINTERP field narrow match: RDNA3 `VinterpMachineInst` stores
  `VDST`, `WAIT_EXP`, `OPSEL`, `CLAMP`, `OP`, `SRC0..SRC2`, and `NEG` in the implementation, and the base `Vinterp` constructor
  remains 64-bit with no literal/DPP size extension in the implementation. The existing VINTERP execution gaps are about interpolation,
...
- Chapter 12 single- and double-address LDS address narrow match: the shared
  ...
  single-address forms in the implementation, while generated `*_2addr_*` bodies use separate scaled
...
- Chapter 11 flat mnemonic narrow match: RDNA3 uses the generated `flat_mnemonic`
  helper to rewrite `flat_` mnemonics to `scratch_` or `global_` based on `SEG` in the implementation, and the `Flat` encoding base installs that owned mnemonic in the implementation.
...
- Chapter 10 MIMG decode-field narrow match: RDNA3 machine structs preserve the
  ...
  NSA address-extension fields in the implementation.
...
- Chapter 10 BVH decode-shell narrow match: generated RDNA3 has distinct
  ...
  operands in the implementation. The execution gap remains `RDNA3-RJ-078`.
...
- Chapter 8 raw `S_LOAD_*` address narrow match: RDNA3 `smem_calculate_address`
  ...
  SGPR/M0/NULL `SOFFSET`, and clears the final two address bits in the implementation, matching the non-buffer formula in section 8.1.1 for the ordinary
  in-range case.
- Chapter 8 SMEM opcode and load-width narrow match: generated RDNA3 SMEM
  ...
  `ScalarMemState` in the implementation.
- Chapter 8 scalar invalidate functional boundary: `S_GL1_INV` invalidates the
  ...
  invalidates scalar L1 in the implementation. The wait-counter and group/isolation semantics remain gaps
...
- Chapter 7.6 VOPD literal-size narrow match: rocjitsu sizes VOPD as 96 bits
  ...
  is an always-literal FMAAK/FMAMK form, in the implementation. This follows the manual's Y-side literal capability and is broader
...
- Chapter 7.6 VOPD ordinary execution boundary: rocjitsu implements the
  ...
  min, ADD_NC, LSHLREV, and AND in the implementation. The
...
- Chapter 7.6 VOPD paired read/write narrow match: `Vopd::execute_impl()`
  computes both X and Y lane results before writing either destination in the implementation, matching the manual's read-old rule for
...
- Chapter 7.4 compact true16 operand narrow match: RDNA3 compact 16-bit source
  operands map `0..127` to `vN.l` and `128..255` to the implementation through
  `packed_16bit_vgpr_source` in the implementation, and compact
  16-bit destinations use the corresponding compact register-reference path
  through `packed_16bit_vgpr_dst`. `RDNA3-RJ-057` is
...
- Chapter 7.4 VOP3 true16 helper narrow match: ordinary generated VOP3 true16
  ...
  source halves and merge the OPSEL-selected destination half in the implementation; representative F16 VOP3 bodies call
  those helpers in the implementation.
- Chapter 3 core-state storage narrow match: `Wavefront` stores PC, live
  ...
  in the implementation. The gaps above concern state fidelity, access
...
- Chapter 3 wave32 mask narrow match: RDNA3 runs as wave32 today
  (`RdnaIsaBase::WF_SIZE = 32` in the implementation), `Wavefront::exec()` masks live lanes in the implementation, and the RDNA3 scalar special-source path
  computes `EXECZ`/`VCCZ` from only active wave lanes in the implementation. `RDNA3-RJ-007` tracks the missing wave64 dispatch selection.
- Chapter 3 branch-condition narrow match: RDNA3 `S_CBRANCH_VCCZ` and
  `S_CBRANCH_EXECZ` test live low-half masks through `wf.vcc()`/`wf.exec()` in the implementation.
- Chapter 3 PC-relative branch narrow match: ordinary `S_BRANCH` and
  ...
  adds `size_` after execution in the implementation,
...
- Chapter 3 direct-PC read narrow match: RDNA3 `S_GETPC_B64`, `S_SWAPPC_B64`,
  and `S_CALL_B64` write the next-instruction PC to their scalar destination in the implementation.
- Chapter 3.3.1 basic scalar-selector narrow match: RDNA3 operand resolution
  ...
  encodings in the implementation; `RDNA3-RJ-016` and
...
- Chapter 3.3.1 VCC execution narrow match: representative RDNA3 VOP2/VOP3
  ...
  `V_DIV_FMAS`, for example in the implementation.
...
- Chapter 3.3.2 zero-VGPR dispatch boundary: the shared register allocator
  refuses zero-count allocations in the implementation;
...
- Chapter 3.3.2 basic VGPR state narrow match: `Wavefront` exposes
  `num_vgprs()` and `vgpr_alloc()` in the implementation, and dispatch fills those fields in the implementation.
...
- Chapter 3.3.2 VOPD narrow match: RDNA3 recognizes VOPD opcodes in the decoder
  and has an executable `Vopd` class in the implementation; `RDNA3-RJ-020` is limited to the source/destination
...
- Chapter 3.3.3 buffer-resource OOB narrow match: `mubuf_calculate_addresses`
  ...
  zeroes those OOB lanes for loads in the implementation.
...
- Chapter 3.3.3 scalar-memory alignment boundary: RDNA3 `smem_calculate_address`
  clears low address bits before SMEM access in the implementation; `RDNA3-RJ-023` is limited to the manual's `SH_MEM_CONFIG`-controlled
...
- Chapter 3.3.4 LDS backing OOB narrow match: `Lds` returns zero for loads and
  drops writes when an address crosses the total simulated LDS backing in the implementation. `RDNA3-RJ-024` and `RDNA3-RJ-025` are about the missing
...
- Chapter 3.3.4 CU/WGP LDS capacity boundary: rocjitsu records the WGP-mode bit,
  ...
  sibling-CU LDS backing in WGP mode in the implementation.
...
- Chapter 3.4.3 M0 storage/operand narrow match: `Wavefront` stores one raw
  ...
  `M0` selector in the implementation /. The inspected explicit write-capable
  `OPR_SDST` constructors are SOP-side, for example
  the implementation,
  the implementation;
  non-SALU consumers such as DS ADDTID read `wf.m0()` as input state in the implementation.
...
- Chapter 3.4.4-3.4.5 NULL/SCC narrow match: RDNA3 scalar reads return zero for
  ...
  example `S_ADDK_I32` writes SCC then writes `SDST` in the implementation, with the NULL no-op handled by the destination resolver.
...
- Chapter 3.4.6 VCC/VCCZ narrow match: `Wavefront` stores raw VCC, scalar
  ...
  `VCCZ` synthesize the wave-size masked live value in the implementation,
  the implementation,
  the implementation. `RDNA3-RJ-012` still covers stale raw `STATUS.VCCZ`, and
...
- Chapter 3.4.8 HWREG boundary: `RDNA3-RJ-011` records the still-incomplete
  RDNA3 `S_GETREG`/`S_SETREG` state coverage, including unsupported
  `FLAT_SCRATCH_LO/HI`, `HW_ID1/2`, `SH_MEM_BASES`, `PC`, and `FLUSH_IB`
  behavior. The rebased ID map itself is correct; this slice keeps the existing
  finding for the unsupported state rather than duplicating it.
...
- Chapter 3.4.9 trap-debug host surface boundary: the KFD shim stores
  ...
  stores debugger exception masks for `AMDKFD_IOC_DBG_TRAP_ENABLE` in the implementation
  /. That is host/API bookkeeping; the command
...
- Chapter 3.5.1 compute `EXEC` initialization narrow match: the command
  ...
  the implementation,
  then writes it with `wf->set_exec(...)` before register initialization in the implementation. Existing tests cover a wave64 grid tail and a 3D tail with
  workgroup offset in the relevant tests. This covers the ordinary compute-dispatch active-lane
...
- Chapter 3.5.2 `FLAT_SCRATCH` initialization narrow match: `Wavefront::reset`
  zeroes `scratch_base_` and `scratch_lane_size_` in the implementation, and
  dispatches with `private_segment_fixed_size > 0` set a per-wave scratch base,
  lane size, and optional `flat_scratch_init` user SGPR pair in the implementation. The remaining scratch fidelity issues are the RDNA3 scalar-selector
...
- Chapter 3.5.3 AMDHSA compute user-SGPR narrow match:
  ...
  private-segment size, and kernarg preload values in the implementation. The `TG_SIZE` and multidimensional X-ID issues are recorded
...
- Chapter 3.5.4 compute workitem-ID VGPR narrow match: rocjitsu reads
  `COMPUTE_PGM_RSRC2_ENABLE_VGPR_WORKITEM_ID` into the dispatch entry in the implementation, computes per-lane local IDs in
  the implementation,
  packs GFX11+ IDs into `v0[9:0]`, `v0[19:10]`, and `v0[29:20]` in the implementation, and writes the requested components in the implementation. Existing dispatch tests cover
  packed-TID component selection in the relevant tests.
...
- Chapter 4.1 `NULL` destination boundary: Chapter 3.4.4 clarifies that when
  ...
  not written, and SCC is updated if the instruction normally updates SCC in the cited manual passage. Rocjitsu's SALU helpers follow that
...
- Chapter 5 ordinary branch narrow match: RDNA3 `S_BRANCH` and the SCC/VCC/EXEC
  ...
  offsets, and update `wf.pc` relative to the next instruction in the implementation, matching the Chapter 5.4 branch formula for those condition classes.
...
- Chapter 5 wait-counter narrow match: RDNA3 `S_WAITCNT` decodes the GFX11
  `EXP/LGKM/VM` fields and sets wait thresholds in the implementation; split `S_WAITCNT_VSCNT`, `S_WAITCNT_VMCNT`,
  ...
  increment/retire the selected counter in the implementation.
...
- Chapter 6 signed-overflow execution narrow match: RDNA3 signed add/sub forms
  ...
  gap; representative bodies include `S_ADD_I32` in the implementation, `S_ADDK_I32`, and signed subtract.
- Chapter 6 conditional, compare, and bit-operation execution narrow match:
  ...
  manual, including `S_CMOVK_I32` in the implementation, `S_CSELECT_*`, and bit-compare helpers. `RDNA3-RJ-047` is limited to def-use metadata.
...
- Chapter 6.9 aperture selector narrow match: once wavefront aperture state is
  ...
  limit selectors from that state for both 32-bit and 64-bit reads in the implementation. Updating that state through architectural
...
- Chapter 7.2.3 floating-point VOP3 modifier narrow match: representative F16,
  ...
  `[0, 1]` clamp in the implementation.
  SIMD helpers also apply floating-point destination modifiers in the implementation. The gaps above are about integer
...
- Chapter 2 shader-padding boundary: generated RDNA3 decodes
  ...
  and `S_SET_INST_PREFETCH_DISTANCE` execute as no-ops in the implementation. Rocjitsu does not model instruction prefetch, so
...
- Chapter 1 architecture-model boundary: rocjitsu has RDNA3 ISA traits, a
  ...
  `RDNA3-RJ-006`.
- Chapter 1 LDS narrow match: the W7900 config reports 64KiB LDS per CU in the test configuration, and WGP-mode LDS capacity is built from a
  paired-CU sum in the implementation, matching the manual's 128KiB-per-WGP / 64KiB-per-workgroup storage
...
- Chapter 1 dispatch/cache boundary: the command processor derives 1D/2D/3D
  workgroup dispatch state and validates LDS capacity in the implementation, and it invalidates CU caches for AQL acquire fences. This is a functional compute-dispatch/cache surface, not a
...
- `V_PK_FMAC_F16` is generated as VOP2, not VOP3P, matching the XML `ENC_VOP2`
  entry in the machine-readable ISA XML; it should not be treated as a missing
...
- MIX execution implements the special selector mapping, treats `NEG_HI` as
  ...
  MIXLO/MIXHI in the implementation.
- DOT2 F32 BF16/F16 helpers apply `CLMP` and use `OPSEL`/`OPSEL_HI` for the
  two packed components in the implementation; broader
...
- Chapter 16.10 WMMA full-`EXEC` execution narrow match: each WMMA definition
  ...
  restores `EXEC` in the cited manual passage. The shared GFX11
  WMMA helpers do not predicate legal base-form result production on
  `wf.exec()`: F32-output writes collected results in the implementation, packed16-output writes every output lane/register, and I32-output writes collected results. Illegal DPP acceptance remains tracked separately in `RDNA3-RJ-096`.
- `DS_BVH_STACK_RTN_B32` is present in the RDNA3 opcode constants and decode
  table in the implementation,
  the implementation /
  the implementation; this is an execution/metadata gap, not a missing decode.
- The generated RDNA3 constructor models the explicit operands and in-out
  `ADDR` path in the implementation, matching the XML operand direction and widths.
- The generated encoding fixture includes `ds_bvh_stack_rtn_b32` in the decode fixtures.
